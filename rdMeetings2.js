...
// reduced dataset to be used as a dataprovider for the meetings list

import roomDataset from './roomDataset.js';

class rdMeetings2 extends roomDataset{ // do not rename without renaming the references to the static properties

    constructor(room){

        // (room, name, isDynamic=false)
        super(room, 'meetingSelection2');
    ...
    updateMeeting(doObj){
        ...
        if (i<0){
            ...
            // process the data
            let newMeetingMod = this.createObject(newMeeting);
    ...
    runMeeting(doObj){
        ...
        let dataSubset = this.createObject(meeting);
    ...
    // takes a liveAthletics meeting and returns the object as needed for dataProviders {name (string), dateFrom (date), dateTo (date), place (string), source (string), hyperlink (string) }
    createObject(laProps){
        return {
            ...
            xMeeting: laProps.xMeeting, // needed for deletion here, actually not for the client 
        }
    }

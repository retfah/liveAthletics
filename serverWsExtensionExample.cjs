...
var wsExt       =   require('./wsExtension.cjs'); // 
...
wss.on('connection', (ws)=>{ // ws is the websocket object
	 ...
	 */
	var requestHandling = async function(request, responseFunc){ // must be arrow function in order that we can use wsProcessor
	...
	// create the wsExtension instance
    // sendingFunc, closingFunction, incomingNoteFunc, incomingRequestFunc, logger=(logLevel, msg)=>{}, opt={}
	const wsProcessor = new wsExt((mess)=>{ws.send(mess);},()=>{ws.close()}, noteHandling, requestHandling, (logLevel, msg)=>{}, {heartbeatMinTimeout: 30});

	// link the websocket events with the wsExtension
    ws.on('message', (mess)=>{
        wsProcessor._onMessage(mess);
    });
	ws.on('close', ()=>{
		wsProcessor.close();

 ...
 *   - one side sends a note; if an ack is requested, the other side immediately answers with an ack (I mean an ack in wsExtension, not an ack on tcp level).
...
//wsExtensionClass: class wsExtensionClass {
	module.exports  = class wsExtensionClass{
		 ...
		 * @param {boolean} opt.openOnConstruct Optional, default=true; is the connection open when the wsExtension is created.
		...
		close(closeWsConnection=false){
			...
			if (this.closing){
				// if it is already closing, there is no need to run the function again. Rerunning this function happens when we call close after heartbeat failure (heartbeat failure --> wsExtension close --> wsConnection close --> wsExtension close)
				return; 
			}
		...
		sendHeartbeat(){
			...
			HB.timeout = setTimeout(()=>{
				...
				// close the wsExtension and the ws connection (will also stop the other timeouts)
				this.close(true);

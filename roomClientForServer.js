/** Difference between roomClientForServer and the regular roomClient:
 * - export default statement
 * - no browser specific stuff, e.g. window.localstorage, window.alert, ...
 * - vues are actually not needed (but still in the code)
 * - when the connection is lost, delete the client class and do not try to reconnect; this will be initiated externally (connectToMainServer)
 *   --> must not only be implemented in rSideChannel, but also here!
 * - allow to set the ID in the constructor
 */
...
export default class roomClient {
    ...
    constructor(v, name, wsHandler, eventHandler, onlineOnly, writing, success, failure, storeInfos=false, rM, datasetName='', writingChangedCB, extraLogger, ID=0, roomEnterOptions=undefined){
        ...
        this.clientName = rM?.data?.clientName ?? this.uuidv4(); 
        ...
        // TODO: this does not work like this; wsHandler does not have those two properties; and probably it also never had them --> check on the client
        // wsHandler is simply the wrong object at the moment; it should be of type socketProcessor2 (or simply have the same methods and properties)
        // socketProcessor only exists on the one side, that initiates the connection --> find a solution that works independent on what side built up the connection
            ...
            this.wsHandler.emitRequest('room', {
                ...
                if (data.type=='full'){
                    ...
                    if (funcsAvail){
                        ...
                        // extended to provide the ID to the function, which is needed in rSideChannelClient, because it will hand over the change to rSideChannel
                        for (let i=0;i<data.data.length; i++){
                            this.functions[data.data[i].funcName](data.data[i].data, data.data[i].ID); // looks quite strange with this triple 'data', but it's correct
                        }
    ...
    /**
     * compared to roomClient, this funciton was extended to provide the ID to the change function, since in rSideChannelCient, we have to pass the change over to rSideChannel (on secondary servers) 
     * @param {*} change 
     */

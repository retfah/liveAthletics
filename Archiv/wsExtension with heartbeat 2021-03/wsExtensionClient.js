 ...
 * - Experiment: shutting down or killing the server (on Windows) is NOT equivalent to a detached cable! It seems that the sockets handling is done by the OS and killing the server means that the reserved port gets free and the socket related to it are closed with a final RST/ACK (i.e. not a normal closing, but a least a message is sent). This leads to a termination of the connection on the client, which is not the same as if the cable was detached. 
 * 
 * wsExtension: 
...
// MAIN TODO: 
// - implement a final version, with the latest understanding of tcp/ws and how connection errors can be detected and handled.
// - reconsider all error codes!
...
//wsExtensionClass: class wsExtensionClass {
	class wsExtensionClass{
		...
		close(){
			...
			for(sHB in this.heartbeat.sent){
		...
		sendHeartbeat(){
			...
			this.logger.log(99, `Ping sent ${nHB}` ); // TODO: comment out
			...
			*/
			...
			var messagetypes = {
				...
				ping: ()=>{
					...
					this.logger.log(99, `Ping ${message.data} arrived. Pong sent.`); // TODO: comment out
				},
				...
				pong: ()=>{
					...
					if (HB===undefined){
						...
						this.logger.log(10, 'Pong message did not match a sent ping. Pong is ignored.')  
						return;
					}
					...
					if (this.heartbeat.nLastArrived+1 != message.data){
						this.logger.log(20, `Pong ${message.data} arrived out of order. Last pong was ${this.heartbeat.nLastArrived}`);
					}
					...
					this.logger.log(99, `Pong ${message.data} arrived within ${rtt} ms.`); // TODO: comment out

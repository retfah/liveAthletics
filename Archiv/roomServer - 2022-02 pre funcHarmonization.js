...
class roomServer{
     ...
     */
    constructor(eventHandler, mongoDb, logger, name, storeReadingClientInfos=false, maxWritingTicktes=-1, conflictChecking=false, dynamicRoom=undefined, reportToSideChannel=true){
    ...
    async initMongoStack(){
        ...
        try {
            ...
            this.collection = this.mongoDB.collection(collectionName);
    ...
    async enter(tabId, wsProcessor, respFunc, opt, session){
        ...
        if (enterRoom){
            ...
            accessRight = this.evaluateRights(tabId, datasetName, opt.enterOptions, session)
        }
     ...
     * @param {object} opt Optional data given to the enter function to be used during the evaluation (TODO: do we really need this)
     ...
     */
     evaluateRights(tabId, datasetName, enterOptions, session){
...
*/
    ...
    func(tabId, respFunc, request){
        ...
        if (request.func in this.functionsReadOnly){
            ...
            respFunc("The function '" + request.func + "' is not available on the server-room.",19);
            return;
        } 
     ...
     */
    _startWriteFunctionServer(func, data, resolve, reject, id=undefined){

        if (!(func in this.functionsWrite)){
            reject({message: "The function '" + func + "' is not available on the server-room.", code:19});
        }
    ...
    wsNoteIncoming(tabId, wsProcessor, arg, opt, session){
        ...
        if (arg=='leave'){
            ...
            let respFunc = (msg, code)=>{
                this.logger.log(20, `A function call that came in as a note could not be processed (code: ${code}, msg: ${msg}). This should actually not happen, but might occur especially for incoming sideChannel bordcasts. ${opt.toString()}`)
            } 
    ...
    finishedFunc(){
        if (this.functionsWorkStack.length>0){
            ...
            if (obj.type=="ws"){
                ...
                this._startWriteFunctionServer(obj.func, obj.data, obj.resolve, obj.reject, obj.id); 
     ...
     */
    async serverFuncWrite(func, data, id=undefined){
        ...
        return new Promise((resolve, reject)=>{
            if (this.busy){
                ...
                this.functionsWorkStack.splice(insertPos,0,{func, data, resolve, reject, type:"server", id}) // "server"-type request
                ...
                // do start the function
                this._startWriteFunctionServer(func, data, resolve, reject, id); 
            }
        })

...
// TODO: check that sendError and log are both used wherever appropriate; sendError should never be without a local log(0) and probably vice versa. 
 ...
 *    - TODO: this should be the same as with NotesAck.
 *      - success: when the requestAck arrives within the timeout
 *      - (fail 0: the requestAck cannot be sent, since the conenction is already closed. In that extremely rare case, the request is not processed at all.)
	...
	class wsExtensionClass{
		...
		close(){
			...
			for (let stamp in this.stackResponse){
				...
				this.stackResponse[stamp].cbAck(2, "Connection closed before responseAck arrived.");
			}
		 ...
		 */
		sendRequestNoAckOld (request, success=(response)=>{}, failure=(errCode, errMsg)=>{}, opt={}){
...
*/
				...
				if (stackObj.sendAck){
					...
					if (stackObj.opt.ackArrived){	
						...
						if (stackObj.opt.ackTimeout>0){
							...
							if (stackObj.opt.ackTimedOut){
								errMsg = `No response arrived within the timeout (${opt.requestTimeout}s)  and also no ack arrived within its own timeout (${opt.ackTimeout}s) of message ${stackObj.message}. `;
								...
								errMsg = `No response arrived within the timeout (${opt.requestTimeout}s), but the ackTimeout did not yet time out (should not happen!?!) of message ${stackObj.message}. `;
			...
			if (opt.ackTimeout>0 && opt.sendAck){
				...
				stackObj.ackTimeoutHandle = setTimeout(()=>{
					let statusMsg = `No ack arrived within the timeout (${opt.timeout}s) of message ${stackObj.message}. `;
		...
		_onMessage(messageRaw){
			...
			*/
			...
			if (!('type' in message)) {
				...
				let msg = 'The message "' + messageRaw + '" has no "type"-property and thus is deleted/dropped.';
				this.logger(0, msg);
				this.sendError(msg);
				return;
			}
			...
			var messagetypes = {
				...
				noteAck: ()=>{
					...
					if (!message.stamp){
						...
						this.logger(0, "noteAck is not valid without stamp!")
					} else {
				...
				request: ()=> {
					...
					if ((message.data!=undefined) && (message.stamp!=undefined) ){
						 ...
						 * @param {callback} cbAck Only if opt.sendAck = true. A callback called when the requestAck arrives. (errCode, errMsg)=>{}. errCode = 0 if the ack arrived successfully. errCode=1 if cbAck is called due to the timeout (if set), errCode=2 if the connection is lost before the ack arrived, errCode=3 if response cannot be sent since the connection is already closed
						 */
						let responseFunc = (response, failureCode=0, opt={}, cbAck=(statusCode, statusMsg)=>{})=>{
							...
							if (opt.sendAck){
								stackObj.ackTimeoutHandle = setTimeout(()=>{
									...
									stackObj.cbAck(1, errMsg) // failure callback
						...
						this.logger(0, "request is not valid without stamp and data properties!")
					}
				...
				requestOld: ()=> {
					...
					if ((message.data!=undefined) && (message.stamp!=undefined) ){
						...
						this.sendError("request is not valid without stamp and data properties!");
						this.logger("request is not valid without stamp and data properties!")
					}
				...
				requestAck: ()=>{
					...
					if (stackObj = this.stackRequest[message.stamp]){
						...
						if (stackObj.opt.ackTimedOut){
							this.logger(0, "Acknowledgement arrived after it has timed out. It is ignored.")
							return;
						}
				...
				response: ()=> {
					...
					if ((message.data!=undefined) && (message.stamp!=undefined)){
						...
						this.logger("Response is not valid without stamp and data properties!");
					}

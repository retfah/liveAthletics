// MAIN TODO: 
//  REMOVE all syn/ack differentiation: Websockets run on TCP; that means, we don not need the syn/ack stuff, as it is guaranteed that the messages do arrive, as long as there is a connection.
...
//wsExtensionClass: class wsExtensionClass {
	module.exports  = class wsExtensionClass{
		...
		/**
		 * 
		 * @param {object} ws The webSocket object (unused --> delete in future!) 
		 ...
		 */
		constructor(ws, sendingFunc, incomingNoteFunc, incomingRequestFunc){
			...
			this.ws = ws; // the websocket-connection (per connection=client) --> TODO: why is ws needed? it is never the same as it depends on the architecture it is running on (e.g node or browser)
		...
		sendNoteAck (note, success=()=>{}, failure=(errCode, errMsg)=>{}, opt={}){
			...
			opt.interval = setInterval(()=>{
				if (stackObj.opt.retryAttempts==stackObj.opt.retryNumber){
					logger.log(99, "The following message could not be sent: " + stackObj.message) // write message to log. Only in debugging-mode, as in general the sending function should decide to log or not in the failure callback
...
/*			opt.interval = setInterval(()=>{
		...
		_onMessage(messageRaw){
			...
			*/
			...
			logger.log(99, "Message recieved per ws: " + messageRaw); // can a  class access a global object
			...
			if (!('type' in message)) {
				...
				logger.log(7, 'The message "' + messageRaw + '" has no "type"-property and thus is deleted/dropped.')
				return;
			}
			...
			var messagetypes = {
				...
				response: function() {
					...
					if (!message.requestStamp){
						self.sendError("response is not valid without requestStamp!");
					} else {
				...
				error: function () {
					logger.log(7, 'A client returned an error for a ws-package: ' + message.data.toString());
				}
			}

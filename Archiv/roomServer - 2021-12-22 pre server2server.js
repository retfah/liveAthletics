...
const ajv 			= 	new Ajv({useDefaults:true}); // check JSON input with schema; apply defaults if available (outside of regular JSON-Schema!)
...
class roomServer{
     ...
     */
    constructor(eventHandler, mongoDb, logger, name, storeReadingClientInfos=false, maxWritingTicktes=-1, conflictChecking=false, dynamicRoom=undefined){
        ...
        // processor: wsProcessor, writing, writingTicketID, name:clientName, storeInfos, views:['', 'roomViewXY', 'roomViewAB', ...]}; // DISCONTINUED views is not used anymore
        ...
        this.offlineWritingClients = {}; // without the processor, as the client is disconnected; hashed sid!
        ...
        // max stack length
        this.maxStackLength = 0; // TODO: set this limit reasonably for the Server used. Maybe no limit is needed at all
    ...
    async initMongoStack(){
        ...
        var initStackDocument = async ()=>{ // it seems like also arrow functions can by async (...wow, cool), then we dont need to bind at the end
    ...
    getSubroom(subroomName){
        ...
        return this.startDynamicSubroom(roomName);
    ...
    enter(tabId, wsProcessor, respFunc, opt, session){
        ...
        if (enterRoom){
            ...
            this.evaluateRights(tabId, datasetName)
        }
        ...
        let listener = ()=>{
            ...
            this.logger.log(99, 'Client '+tabId+' got disconnected from room '+this.name + ' without properly leaving first.');
            ...
            if (writing){
                ...
                //this.offlineWritingClients[tabIdHash] = client;
                this.offlineWritingClients[this.createHash(client.session.id)] = client; // 2021-01: use the sid to store the offline writing client (since the tabId would change e.g. on reload (it does not change when the same tab reconnects witout reload))
                this.storeOfflineWritingClients();
            }
        ...
        this.clients[tabId] = {processor: wsProcessor, writing:writing, writingTicketID:writingTicketID, name:name, storeInfos:storeInfos, tabIdHash:tabIdHash, sidHash:sidHash, datasetName:datasetName, session:session};
        ...
        if (dataset){
            ...
            if (opt.ID){
                ...
                if (i>=0){
                    ...
                    data.type = 'full';
                    data.data = this.getPersonalizedData(this.clients[tabId]); 
                }
        ...
        this.logger.log(99, "Successfully entered the room '" + this.name + "'. tabId: "+ tabId + " Name: "+ name)
     ...
     */
    evaluateRights(tabId, datasetName){
...
*/
    ...
    requestWritingTicket(tabId, respFunc){
        ...
        if (this.maxWritingTicktes!=-1 && this.writingTickets.length >= this.maxWritingTicktes){
            ...
            this.clients[tabId].writing = true;
            this.clients[tabId].writingTicketID = writingTicketID;
    ...
    func(tabId, respFunc, request){
        ...
        if (request.func in this.functionsReadOnly){
            ...
            this.functionsReadOnly[request.func](request.data).then((ret)=>{
                ...
                if (developMode){
                    ...
                    if(!(this.ajv.validate(schemaFail, ret))){
                        let text = "Error: The error-object returned from the room-function '"+ JSON.stringify(request.func) +"' in room '"+ this.name +"' does not fulfill the failure-schema.";
            ...
            if (this.busy){
                ...
                this.logger.log(67, "Need to put a change on the stack since multiple changes were requested at the same time. Client " + tabId + " with a request to the '" + request.func + "'-function has to wait...");
            } else {
     ...
     */
    _startWriteFunctionServer(func, data, resolve, reject){

        if (!(func in this.functionsWrite)){
            reject({message: "The function '" + func + "' is not available on the server-room.", code:12});
        }

        this.functionsWrite[func](data).then((ret)=>{
            //ret = {isAchange, response, doObj, undoObj, preventBroadcastToCaller};

            // create the ID and broadcast to the clients: 
            if (ret.isAchange){
                this._processChange(ret.doObj, ret.undoObj)
            }
    ...
    _startWriteFunction(tabId, respFunc, request, ID=''){
        // old: sync
        //this.functions[request.func](tabId, request.data, respFunc)
        ...
        this.functionsWrite[request.func](request.data).then((ret)=>{
            ...
            var serverProcessing =  async (ret)=>{
                if (developMode){
                    ...
                    if(!(this.validateSuccess(ret))){
                        let text = "Error: The object ("+JSON.stringify(ret)+") returned from the room-function '"+ request.func +"' in room '"+ this.name +"' does not fulfill the success-schema. schema-error: " + this.ajv.errorsText();
                        this.logger.log(3, text)
                        respFunc("Error on the server: " + text, 11)
                        return;
                    }
            ...
            return serverProcessing(ret).catch((err)=>{
                ...
                let text = "Error in processing the change from the room-function '"+ request.func +"' in room '"+ this.name +"'. Error: " + err.toString();
                this.logger.log(3, text)
                respFunc("Error on the server: " + text, 11)
                return;
            })
            ...
            if (developMode){
                ...
                if(!(this.validateFail(err))){
                    let text = "Error: The error-object returned from the room-function '"+ request.func +"' in room '"+ this.name +"' does not fulfill the failure-schema. Error: " + JSON.stringify(err);
                    this.logger.log(3, text)
                    respFunc("Error on the server: " + text, 11)
                    return;
                }
    ...
    // TODO: the structure of the data is not ideal: the roomName (and in the view also the viewName) should not be a property of the obj itself, but rathe rof an additional layer of data (or is it already like that?)
    /**
    ...
    wsNoteIncoming(tabId, wsProcessor, arg, opt, session){
        ...
        if (arg=='leave'){
            ...
            logger.log(75,`This argument ${arg} is not a valid argument.`);
        }
    }
     ...
     */
    wsRequestIncoming(tabId, wsProcessor, responseFunc, arg, opt, session){
        // call the respective function on this room
        if (arg=='enter'){
            ...
            if (!this.isClient(tabId)){
                throw {msg:'The client is not in the room!' , code: 99};
            } else {
            ...
            if (n===''){
                ret.data = this.getPersonalizedData(client);
            } else {
    ...
    finishedFunc(){
        if (this.functionsWorkStack.length>0){
            ...
            if (obj.type=="ws"){
                ...
                this._startWriteFunctionServer(obj.func, obj.data, obj.resolve, obj.reject); 
            }
        }else{
            this.busy = false;
        }
     ...
     */
    async serverFuncReadOnly(func, data){
        return this.functionsReadOnly[func](data)
    }

    /**
     * Call a serverFunction taht writes data. To be used by other server functions/other rooms to get data from this room.
     * @param {string} func The name of the function
     * @param {any} data The data used for processing the request
     * @returns Promise / The response of the function
     */
    async serverFuncWrite(func, data){

        return new Promise((resolve, reject)=>{
            if (this.busy){
                this.functionsWorkStack.push({func, data, resolve, reject, type:"server"}) // "server"-type request
                ...
                // do start the function
                this._startWriteFunctionServer(func, data, resolve, reject); 
            }
        })

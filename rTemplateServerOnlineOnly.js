...
class r<%= roomUpperPlural %> extends roomServer{
    ...
    async delete<%= roomUpper %>(data){
        ...
        if (valid){
            ...
            let [ind, <%= room %>] = this.findObjInArrayByProp(this.data, '<%= primary %>', data)
            ...
            [ind, ] = this.findObjInArrayByProp(this.data, '<%= primary %>', data) // must be reqpeated, since the index could have changed due to the async call.
            if (ind>=0){
    ...
    async update<%= roomUpper %>(data){
        ...
        if (valid) {
            ...
            let [i, o] = this.findObjInArrayByProp(this.data, '<%= primary %>', data.<%= primary %>);
            if (i<0){

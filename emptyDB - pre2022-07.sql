...
CREATE TABLE IF NOT EXISTS `baseDisciplines` (
  ...
  `relay` BIT NOT NULL DEFAULT False COMMENT 'relays are a special discipline since it requires multiple athletes and therefore has a separate inscription',
...
CREATE TABLE IF NOT EXISTS `events` (
  ...
  `onlineId` INT UNSIGNED NOT NULL DEFAULT '0',
  `date` DATETIME NULL,
...
CREATE TABLE IF NOT EXISTS `starts` (
  ...
  `inBase` VARCHAR(25) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `sites` (
  ...
  `isTrack` BIT NOT NULL DEFAULT false,
  PRIMARY KEY (`xSite`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
...
CREATE TABLE IF NOT EXISTS `resultsTrack` (
  ...
  `timeRounded` SMALLINT UNSIGNED NOT NULL COMMENT 'rounded time, digits and rounding defined by the discipline',
  `rank` TINYINT UNSIGNED NOT NULL COMMENT 'the ranking made by the finish-judge must be used and not the ranking given by the time!',
...
CREATE TABLE IF NOT EXISTS `resultsTechWind` (
  ...
  `result` SMALLINT NOT NULL DEFAULT 0 COMMENT 'in cm',
  `attempt` TINYINT UNSIGNED NOT NULL COMMENT 'which attempt (start from 1)',
  `wind` SMALLINT NOT NULL DEFAULT 0 COMMENT 'unfortunately Wind must appear twice in the column-name\n\nmust be signed (+-)!\nin m/s\n',

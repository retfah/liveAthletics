...
// /* 				highlightedMsg:function(){
// 					// if the highlighted msg changes, we have to recreate the JSON with the reverted stack (if available) 
...
<div v-else-if="msg.type=='outdatedRollback'">
	 ...
	 * @param {object} deletedStack Optional, The stack that was reverted and is provided here for download
	 */
	messageOutdatedRollbackError(room, requestInfo, request, deletedStack){
		let msgLength = this.data.messages.push({
			type: "outdatenRollback",
			time: new Date(),
			room: room,
			info: requestInfo,
			request: request,
